    ...
    def _extract_audio(video_path: Path, target_duration: float) -> dict[str, torch.Tensor | int] | None:
        ...
        try:
            ...
            if current_samples > target_samples:
                ...
                logger.debug(f"Padded audio to {target_duration:.2f} seconds for {video_path}")
...
def compute_latents(  # noqa: PLR0913, PLR0915
    ...
    # Filter out already-computed entries so the dataloader never loads them
    data_root = dataset.dataset_file.parent
    original_count = len(dataset)
    keep_indices = []
    for idx in range(len(dataset.main_media_paths)):
        rel = dataset.main_media_paths[idx].relative_to(data_root).with_suffix(".pt")
        video_done = (output_path / rel).exists()
        audio_done = not with_audio or (audio_output_path / rel).exists()
        if not (video_done and audio_done):
            keep_indices.append(idx)

    if len(keep_indices) < original_count:
        skipped = original_count - len(keep_indices)
        ...
        logger.info(f"Skipping {skipped} already-computed files, {len(dataset)} remaining")

    if len(dataset) == 0:
        logger.info("All files already computed, nothing to do")
        return
            ...
            for i in range(len(batch["relative_path"])):
                ...
                if with_audio:
                    ...
                    if audio_batch is not None:
                        ...
                        # Encode audio (returns None for unsupported channel counts)
                        with torch.inference_mode():
    ...
    if with_audio:
        logger.info(
            ...
            f"{audio_skip_count} videos without audio or unsupported channels (skipped)"
        )
...
def encode_audio(
    ...
    Returns None for unsupported audio formats (e.g. >2 channels / 5.1 surround).
    """
    ...
    if waveform.shape[1] == 1:
        ...
        logger.warning(f"Skipping audio with {waveform.shape[1]} channels (only mono/stereo supported)")
        return None
