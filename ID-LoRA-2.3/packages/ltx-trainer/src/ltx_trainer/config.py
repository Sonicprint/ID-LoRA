...
class ValidationConfig(ConfigBaseModel):
    ...
    mask_cross_attention_to_reference: bool | None = Field(
        default=None,
        description="If set, overrides training strategy for validation only: block video from attending "
        "to reference audio in a2v cross-attention. None = use strategy value.",
    )

    mask_ref_audio_to_text: bool | None = Field(
        default=None,
        description="If set, overrides training strategy for validation only: block reference audio from "
        "attending to text. None = use strategy value.",
    )
    ...
    speaker_metric: Literal["ecapa_tdnn", "wavlm_ecapa", "none"] | None = Field(
        default="wavlm_ecapa",
        description="Speaker similarity metric: 'wavlm_ecapa' (WavLM+ECAPA-TDNN SOTA), "
        "'ecapa_tdnn' (SpeechBrain), or 'none'/null to disable.",
    )
...
class LtxTrainerConfig(ConfigBaseModel):
    ...
    def validate_strategy_compatibility(self) -> "LtxTrainerConfig":
        ...
        # Check that LoRA config is provided when using IC-LoRA strategies
