...
class GenerationConfig:
    ...
    reference_audio: Tensor | None = None  # For audio IC-LoRA: waveform [C, samples]
    reference_audio_sample_rate: int = 16000
    audio_only_ic: bool = False  # Audio-only IC: video is standard T2V, only audio uses reference
    ...
    use_negative_ref_positions: bool = False  # Negate reference temporal positions for audio-ref-only IC-LoRA
    mask_cross_attention_to_reference: bool = False  # Block video from attending to reference audio in a2v attention
...
class ValidationSampler:
    ...
    def __init__(
        ...
        Args:
            ...
            audio_processor: Optional AudioProcessor (for waveform-to-spectrogram conversion)
    ...
    def _generate_with_audio_reference(
        ...
        """Generate with audio-only IC conditioning.
        Video uses standard T2V (no video reference), audio uses reference concatenation.
        """
        v_ctx_pos, a_ctx_pos, v_ctx_neg, a_ctx_neg = self._get_prompt_embeddings(config, device)

        generator = torch.Generator(device=device).manual_seed(config.seed)

        # Standard T2V video (no video reference IC)
        video_tools = self._create_video_latent_tools(config)
        video_clean_state = video_tools.create_initial_state(device=device, dtype=torch.bfloat16)
        ...
        # Audio state with reference conditioning (IC style)
        audio_tools = self._create_audio_latent_tools(config) if config.generate_audio else None
        ref_audio_seq_len = 0

        if audio_tools is not None and config.reference_audio is not None:
            ...
            if config.use_negative_ref_positions:
                logger.info("Using NEGATIVE AUDIO reference positions")
                hop_length = 160
                audio_latent_downsample_factor = 4
                audio_sample_rate = 16000
                time_per_latent = hop_length * audio_latent_downsample_factor / audio_sample_rate
        ...
        # Create cross-attention masks if configured
        a2v_cross_attention_mask = None
        audio_context_attn_mask = None

        if ref_audio_seq_len > 0:
            video_seq_len = video_state.latent.shape[1]
            target_audio_seq_len = audio_state.latent.shape[1] - ref_audio_seq_len if audio_state is not None else 0
