...
class LtxvTrainer:
    def __init__(self, trainer_config: LtxTrainerConfig) -> None:
        ...
        self._face_metric = None
        self._clap_metric = None
        self._init_wandb()
    ...
    def _load_models(self) -> None:
        ...
        # Load audio VAE encoder for reference audio conditioning in audio-only IC strategies
        ...
        for prompt_idx, prompt in enumerate(self._config.validation.prompts):
            ...
            if use_reference_videos:
                ...
                if generate_audio and self._audio_encoder is not None:
                    import torchaudio
                    try:
                        waveform, sr = torchaudio.load(str(ref_video_path))
                        reference_audio = waveform
                        reference_audio_sample_rate = sr
                    except Exception as e:
                        logger.warning(f"Could not extract audio from reference video {ref_video_path}: {e}")

            # Determine strategy flags for validation
            audio_only_ic = self._config.training_strategy.name in ("audio_ic", "audio_ic_masked", "audio_ref_only_ic")
            use_negative_ref_positions = getattr(self._config.training_strategy, "use_negative_ref_positions", False)

            # Resolve mask flags: validation config overrides strategy defaults
            strategy = self._config.training_strategy
            _mask_xattn = getattr(strategy, "mask_cross_attention_to_reference", False)
            _mask_text = getattr(strategy, "mask_reference_from_text_attention", False)
            val_cfg = self._config.validation
            mask_cross_attention_to_reference = (
                val_cfg.mask_cross_attention_to_reference if val_cfg.mask_cross_attention_to_reference is not None
                else _mask_xattn
            )
            mask_ref_audio_to_text = (
                val_cfg.mask_ref_audio_to_text if val_cfg.mask_ref_audio_to_text is not None
                else _mask_text
            )
            ...
            if self._clap_metric is not None:
                ...
                if caption:
                    try:
                        ...
                        logger.warning(f"CLAP env similarity failed for {gen_path.name}: {e}")
                    ...
                    try:
                        ...
                        logger.warning(f"CLAP speaking similarity failed for {gen_path.name}: {e}")
